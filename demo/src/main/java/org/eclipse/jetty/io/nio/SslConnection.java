    ...
    public Connection handle() throws IOException {
        try {
            ...
            while (progress) {
                ...
                _logger.info("SQ1: connection {} {} handle {} progress={}",_connection, _session, this, progress);

                _logger.info("{} handle {} progress={}", _session, this, progress);
            }
        } finally {
            releaseBuffers();
            _logger.info("SQ2: break handle {}",_connection);
        ...
        try {
            ...
            while (progress) {
                ...
                try {
                    ...
                    _logger.info("{} {} {} filled={}/{} flushed={}/{}", _session, this, _engine.getHandshakeStatus(), filled, _inbound.length(), flushed, _outbound.length());
                }
...
//                            progress = true;
                        //FixMe::suqnuan
                        if (toFlush.hasContent() && _outbound.space() > 0) {
                            SSLEngineResult result = transResult(toFlush);
                            _logger.info("SQ:{} wrap {} {} consumed={} produced={},progress={}",
                                    _session,
                                    result.getStatus(),
                                    result.getHandshakeStatus(),
                                    result.bytesConsumed(),
                                    result.bytesProduced(), progress);
                            if (sqWrap(result)) {
                                _logger.info("SQ:{} progress={}", _session, progress);
                                progress = true;
                            } else {
    ...
    private synchronized SSLEngineResult transResult(final Buffer buffer) throws IOException {
    ...
    private synchronized boolean sqWrap(SSLEngineResult result) throws IOException {
        _logger.info("SQ:endp inputS={} endp outputS={}", _endp.isInputShutdown(), _endp.isOutputShutdown());
        switch (result.getStatus()) {
            ...
            case CLOSED:
                _logger.info("wrap CLOSE {} {}", this, result);
                if (result.getHandshakeStatus() == HandshakeStatus.FINISHED)
                    _endp.close();
                break;
