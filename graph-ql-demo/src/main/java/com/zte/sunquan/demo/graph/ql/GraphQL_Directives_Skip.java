 ...
 */
public class GraphQL_Directives_Skip {
    ...
    public static void main(String[] args) {
        ...
        GraphQLObjectType queryType = newObject()
                ...
                .field(newFieldDefinition().type(userType).name("user")
                        .argument(newArgument().name("notShow").type(GraphQLBoolean).defaultValue(false)
                        )
                        .dataFetcher(env -> {
                            return user;
                        }))
        ...
        Map<String, Object> result = graphQL.execute("{user{name @skip(if: true),gender,intro}}").getData();
        System.out.println(result);
        Map<String, Object> variable = Maps.newHashMap();
        variable.put("notShow", Boolean.TRUE);
        ExecutionInput executionInput = ExecutionInput.newExecutionInput().variables(variable)
                .query("query userQuery($notShow:Boolean!){user{name @skip(if: $notShow),gender,intro}}").build();
